...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the scoped Codex task history described in `runtime.md`. Read only task IDs or rollout files already matched to the requested workspace; that reads unrelated private chats. Walk the log against what actually happened:
