...
Before spawning investigators, discover callable connectors from the live Codex tool inventory. Use tool search when exposed; do not assume a local MCP descriptor directory exists.
...
- read-only assignment: `false` (agent mode). **Do not use readonly/Ask mode.** It strips MCP access, which disables MCP-backed investigators entirely. The source control investigator would be safe in readonly, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- read-only assignment: `false` (agent mode). The synthesizer's quality check spot-verifies citations, which can require MCP access. Readonly/Ask mode strips MCPs and defeats that.
