...
Read the Codex history procedure in `runtime.md`. Prefer app task listing/reading tools; select only tasks in the requested workspace and time window. For a supplied local rollout, read `session_meta` / `turn_context` before its messages to verify scope. A rollout is an event stream, not one chat message per line. Do not search unrelated conversations.
