...
Use [the retained team protocol](references/retained-team.md). The default worker is the current main thread (Terra medium preferred when selectable); it owns investigation, design, acceptance, implementation and verification without new sessions at each phase. Bind one independent Sol medium reviewer at the first review and resume that same reviewer for repairs. Do not spawn an acceptance author or an implementation replacement simply because the phase changed. This role policy overrides actor/model instructions in all bundled playbooks and linked skills, including mandatory implementation delegation, architect arenas and PR model handoffs. Ordinary tasks use no extra investigator, planner or implementer. Only explicitly requested eval/program workflows retain their registered panels.

Apply [conditional design grilling](references/design-grilling.md) before freezing acceptance. If the requirement and existing implementation path are clear, record `assess-design` with no unresolved questions and proceed without grilling. Otherwise the same worker/reviewer resolve technical questions in dependency order; only product-policy, authority and irreversible choices go to the user, reusing existing decisions where available. Map every agreed implementation decision to requirement IDs, implementation locations and verification methods. Do not leave confirmed decisions only in a conversation transcript.
...
2. Perform the active action with the retained worker and actual host tools. For browser verification, delegate execution through [Cursor ACP browser verification](references/cursor-browser-verification.md); the worker retains evidence validation and completion ownership. Freeze requirement IDs once with the worker; map verification evidence to those IDs. The main worker compares design alternatives itself; source delegation clauses are adapted as described in the execution policy. The reviewer begins independently, then receives the review context on every resumed review. A model recommendation is not evidence that the model ran.
...
For review findings use `team` to record stable IDs, existing requirement IDs, affected files, evidence and `basis`. Re-review focuses on agreed design omissions and regressions introduced by this change. Existing out-of-scope issues and improvements go to followups without repair or renewed approval. Newly discovered dependencies that prevent safe operation require a recorded scope disposition before repair. Worker marks required findings fixed; only the retained reviewer resolves. Use `repair-finding` for a guarded return to the responsible stage. Environment and formatting failures return to verification, never a fresh acceptance-author run. `next` includes design decisions, prior findings and reusable/stale checks; pass them with the actual diff. Reuse only unchanged declared inputs; include shared callers, configuration and dependency manifests when relevant. All required findings and independent approval must be satisfied before completion.
