...
This skill orchestrates three others: an inline mining pass (see step 1), the bundled `create-skill` workflow (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with the available Codex question tool (unless they already said "update my skill" or similar):
...
Locate the in-scope Codex tasks using the history procedure in `runtime.md`. Select by workspace metadata and the requested time window before reading content. Supply exact task IDs or verified rollout paths to delegates. Never scan other workspaces to infer preferences.
...
Mining misses intent that hasn't come up yet. Use the the available Codex question tool tool (structured multi-choice) rather than asking the user to type from scratch. Lower cognitive load, higher hit rate.

Use the actual question tool schema for option counts and selection support. Do not invent multi-select arguments. Start broad ("Which areas matter most?"), then follow up on selected areas with specific options. After the structured rounds, one free-form chat question catches anything the options missed.
...
Use the bundled `create-skill` workflow skill to author the skill. Placement:
...
- Keep normal Codex discovery unless the user requests explicit-only invocation. For that request, use `policy.allow_implicit_invocation: false` in `agents/openai.yaml`; do not emit Cursor mode metadata.
...
- the bundled `create-skill` workflow skill: skill authoring process and writing guidelines.
