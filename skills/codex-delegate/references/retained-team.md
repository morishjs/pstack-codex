...
Browser execution defaults to [Cursor ACP](cursor-browser-verification.md). Keep its session and transcript as verification provenance under the worker's checks. Cursor is an external execution provider, not the independent Sol reviewer or a native panel assignment. Reuse its session across retests; the main worker still owns repairs and evidence acceptance.

Only for an explicitly requested eval, autopilot or standing-program panel, register `{"type":"assign","role":"specialist","agentId":"...","sourceClause":"eval/candidates","reason":"User requested independent model candidates"}` with host-assignment and source/need evidence. Resume registered participants when the same role is needed again. `next.panelRequirement` identifies panel gates: supply `data.panel={expectedCount,memberIds}` from the actual source configuration and matching registered participants. The minimum is only a structural floor; do not lower a larger configured panel. Ordinary feature design is performed by the main worker and has no participant panel requirement. Eval/swarm required panels cannot omit participant records. Candidate participants cannot serve a differently scoped judge panel through the same assignment.

The execution policy overrides original source instructions to create investigators, design arenas, implementation owners or PR model handoffs. Ordinary tasks keep the main worker and create one reviewer only when independent review is required; Cursor remains the browser provider. Preserve actual host concurrency limits. This is not a universal maximum of two agents; it makes additional participants and their reasons visible rather than spawning per phase by default. Unavailable sessions are a concrete recovery problem, not permission to silently replace identities. Host-assignment files and declared inputs are auditable evidence, not an OS-level attestation: native actions and semantic correctness still require inspection.
